...
public func demoServer(publicDir: String) -> SwifterHttpServer {
    
    print(publicDir)
    
    let server = SwifterHttpServer()
    
    server["/public/:path"] = SwifterHttpHandlers.shareFilesFromDirectory(publicDir)
    
    server["/files/:path"] = SwifterHttpHandlers.directoryBrowser("/")
    ...
    server.POST["/login"] = { r in
        ...
        return SwifterHttpResponse.OK(.Html(formFields.map({ "\($0.0) = \($0.1)" }).joinWithSeparator("<br>")))
    }
    ...
    server["/raw"] = { r in
        return SwifterHttpResponse.RAW(200, "OK", ["XXX-Custom-Header": "value"], { $0.write([UInt8]("test".utf8)) })
    }
    ...
    server["/stream"] = { r in
        return SwifterHttpResponse.RAW(200, "OK", nil, { w in
            for i in 0...100 {
    ...
    server["/websocket-echo"] = SwifterHttpHandlers.websocket({ (session, text) in
