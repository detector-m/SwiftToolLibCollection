...
extension String {
    ...
    public func trim() -> String {
        var scalars = self.unicodeScalars
        while let _ = scalars.first?.asWhitespace() {
            scalars.removeFirst()
        }
        while let _ = scalars.last?.asWhitespace() {
            scalars.removeLast()
        }
    ...
    public func removePercentEncoding() -> String {
        ...
        while let scalar = scalars.popFirst() {
            if scalar == "%" {
                ...
                let second = scalars.popFirst()
                if let first = first?.asAlpha(), second = second?.asAlpha() {
                    decodeBuffer.append(first*16 + second)
                }
                else {
                    ...
                    if let second = second {
                        output.append(Character(second))
                    }
                }
            }
            else {
