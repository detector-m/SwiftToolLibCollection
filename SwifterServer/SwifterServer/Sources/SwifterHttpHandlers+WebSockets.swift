//
//  SwifterHttpHandlers+WebSockets.swift
//  SwifterServer
//
...
extension SwifterHttpHandlers {
    public class WebSocketSession {
        public enum Error: ErrorType {
            case UnknownOpCode(String), unMaskedFrame
        }
        public enum OpCode {
        ...
        private let socket: SwifterSocket
        
        public init(_ socket: SwifterSocket) {
            self.socket = socket
        }
        ...
        private func writeFrame(data: ArraySlice<UInt8>, _ op: OpCode, _ fin: Bool = true) {
            ...
            let maskAndLength = encodeLengthAndMasFlag(UInt64(data.count), false)
            do {
                try socket.writeUInt8([finAndOpCode])
                try socket.writeUInt8(maskAndLength)
        ...
        private func encodeFinAndOpCode(fin: Bool, op: OpCode) -> UInt8 {
            var encodeByte = UInt8(fin ? 0x80 : 0x00)
            switch op {
            case .Continue:
                encodeByte |= 0x00 & 0x0f
            case .Text:
                encodeByte |= 0x01 & 0x0f
            case .Binary:
                encodeByte |= 0x02 & 0x0f
            case .Close:
                encodeByte |= 0x08 & 0x0f
            case .Ping:
                encodeByte |= 0x09 & 0x0f
            case .Pong:
                encodeByte |= 0x0a & 0x0f
            }
            return encodeByte
        }
        
        private func encodeLengthAndMasFlag(len: UInt64, _ masked: Bool) -> [UInt8] {
            let encodedLength = UInt8(masked ? 0x80: 0x00)
            var encodedBytes = [UInt8]()
            switch len {
            case 0...125:
                encodedBytes.append(encodedLength | UInt8(len))
            case 126...UInt64(UINT16_MAX):
                encodedBytes.append(encodedLength | 0x7e)
                encodedBytes.append(UInt8(len >> 8))
                encodedBytes.append(UInt8(len & 0xff))
            default:
                encodedBytes.append(encodedLength | 0x7f)
        ...
        public func readFrame() throws -> Frame {
            ...
            switch opc {
            case 0x00:
                frm.opcode = OpCode.Continue
            case 0x01:
                frm.opcode = OpCode.Text
            case 0x02:
                frm.opcode = OpCode.Binary
            case 0x08:
                frm.opcode = OpCode.Close
            case 0x09:
                frm.opcode = OpCode.Ping
            case 0x0a:
                frm.opcode = OpCode.Pong
    ...
    public class func websocket(text: ((WebSocketSession, String) -> Void)?, _ binary: ((WebSocketSession, [UInt8]) -> Void)?) -> (SwifterHttpRequest -> SwifterHttpResponse) {
        return {
            r in
            guard r.headers["upgrade"] == "websocket"
                else {
                    return .BadRequest(.Text("Invalid value of 'Upgrade' header: \(r.headers["upgrade"])"))
            }
            guard r.headers["connection"] == "Upgrade"
                else {
                    return .BadRequest(.Text("Invalid value of 'Connection' header: \(r.headers["connection"])"))
            }
            guard let secWebSocketKey = r.headers["sec-websocket-key"]
                else {
                    return .BadRequest(.Text("Invalid value of 'Sec-Websocket-Key' header: \(r.headers["sec-websocket-key"])"))
            }
            let protocolSessionClosure: (SwifterSocket -> Void) = {
                socket in
            ...
            return SwifterHttpResponse.SwitchProtocols(headers, protocolSessionClosure)
        }
    }
}