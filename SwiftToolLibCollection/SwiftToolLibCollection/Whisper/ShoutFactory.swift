...
public func Shout(announcement: Announcement, to: UIViewController, completion: (() -> Void)? = {}) {
    shout.craft(announcement, to: to, completion: completion)
}

public class ShoutView: UIView {
    public struct Dimensions {
        public static let indicatorHeight: CGFloat = 6.0
        public static let indicatorWidth: CGFloat = 50.0
    ...
    public private(set) lazy var titleLable: UILabel = {
        let label = UILabel()
    ...
    public private(set) lazy var tapGestureRecongizer: UITapGestureRecognizer = {
        [unowned self] in
        let gesture = UITapGestureRecognizer()
        gesture.addTarget(self, action: #selector(ShoutView.handleTapGestureRecongnizer))
    ...
    public override init(frame: CGRect) {
        ...
        [indicatorView, imageView, titleLable, subtitleLabel, gestureContainer].forEach({ backgroundView.addSubview($0) })
        clipsToBounds = false
        ...
        addGestureRecognizer(tapGestureRecongizer)
    ...
    // MARK: - Configuration
    public func craft(announcement: Announcement, to: UIViewController, completion: (() -> Void)?) {
    ...
    public func configureView(announcement: Announcement) {
        ...
        titleLable.text = announcement.title
        subtitleLabel.text = announcement.subtitle ?? ""
        
        [titleLable, subtitleLabel].forEach {
            $0.sizeToFit()
        }
        
        if imageView.image == nil {
            Dimensions.textOffset = 18
        }
    ...
    public func shout(to controller: UIViewController) {
        guard let controller = controller.navigationController
            else {
    ...
    public func setupFrames() {
        ...
        imageView.frame = CGRect(x: Dimensions.imageOffset, y: (Dimensions.height - Dimensions.imageSize) / 2 + offset, width: Dimensions.imageSize, height: Dimensions.imageSize)
        titleLable.frame.origin = CGPoint(x: Dimensions.textOffset, y: imageView.frame.origin.y + 3)
        subtitleLabel.frame.origin = CGPoint(x: Dimensions.textOffset, y: CGRectGetMaxY(titleLable.frame) + 2.5)
        
        if let text = subtitleLabel.text where text.isEmpty {
            titleLable.center.y = imageView.center.y - 2.5
        }
    ...
    @objc private func handleTapGestureRecongnizer() {
        guard let announcement = announcement
            else {
                return
        }
        announcement.action?()
