//
//  WhisperFactory.swift
//  SwiftToolLibCollection
//
//  Created by Riven on 16/5/18.
...
public func Silent(controller: UINavigationController, after: NSTimeInterval = 0) {
    whisperFactory.silentWhisper(<#T##controller: UINavigationController##UINavigationController#>, after: <#T##NSTimeInterval#>)
}
...
class WhisperFactory: NSObject {
    ...
    func silentWhisper(controller: UINavigationController, after: NSTimeInterval) {
        ...
        if whisperSubview == nil { return }
        
        whisperView = whisperSubview
        delayTimer.invalidate()
        delayTimer = NSTimer.scheduledTimerWithTimeInterval(after, target: self, selector: #selector(WhisperFactory.delayFired(_:)), userInfo: nil, repeats: false)
    }
    ...
    func showView() {
        ...
        UIView.animateWithDuration(AnimationTiming.movement, animations: { 
            ...
            for subview in self.whisperView.transformViews {
                ...
                _ in
                self.delayTimer = NSTimer.scheduledTimerWithTimeInterval(1.5, target: self, selector: #selector(WhisperFactory.delayFired(_:)), userInfo: nil, repeats: false)
        })
    }
    ...
    func changeView(message: Message, action: WhisperAction) {
        ...
        presentTimer = NSTimer.scheduledTimerWithTimeInterval(AnimationTiming.movement * 1.1, target: self, selector: #selector(WhisperFactory.presentFired(_:)), userInfo: array, repeats: false)
    }
    ...
    func presentFired(timer: NSTimer) {
        guard let userInfo = timer.userInfo, title = userInfo["title"] as? String, textColor = userInfo["textColor"] as? UIColor, backgroundColor = userInfo["backgroundColor"] as? UIColor, actionString = userInfo["action"] as? String
            else {
                return
        }
        ...
        if let imageArray = userInfo["images"] as? [UIImage]? {
            images = imageArray
        }
    ...
    func moveControllerViews(down: Bool) {
        guard let visibleController = navigationController.visibleViewController where Config.modifyInset
            else {
                return
        }
    ...
    func performControllerMove(viewController: UIViewController) {
        guard Config.modifyInset
            else { return }
        if let tableView = viewController.view as? UITableView where viewController is UITableViewController {
            tableView.contentInset = UIEdgeInsetsMake(tableView.contentInset.top + edgeInsetHeight, 0, 0, 0)
        }
        else if let collectionView = viewController.view as? UICollectionView where viewController is UICollectionViewController {
    ...
    func orientationDidChange() {
        for subview in navigationController.navigationBar.subviews {
            ...
            whisper.frame = CGRect(x: whisper.frame.origin.x, y:maximumY, width: UIScreen.mainScreen().bounds.width, height: whisper.frame.size.height)
            whisper.setupFrames()
        }
    }
}
    ...
    func navigationController(navigationController: UINavigationController, willShowViewController viewController: UIViewController, animated: Bool) {
        ...
        for subview in navigationController.navigationBar.subviews {
            if subview is WhisperView {
                navigationController.navigationBar.bringSubviewToFront(subview)
            }
