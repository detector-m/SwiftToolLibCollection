...
public class SKCaptionView: UIView {
    ...
    public override func sizeThatFits(size: CGSize) -> CGSize {
        guard let text = photoLabel.text
            else {
                return CGSize.zero
        }
        guard photoLabel.text?.characters.count > 0
            else {
                return CGSize.zero
        }
        ...
        let textSize = attributedText.boundingRectWithSize(CGSize(width: width, height: height), options: NSStringDrawingOptions.UsesLineFragmentOrigin, context: nil).size
