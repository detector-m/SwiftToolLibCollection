...
public class NetTCPSSL: NetTCP {
    ...
    public override func initSocket() {
        super.initSocket()
        guard self.sslCtx == nil
            else {
                return
        }
        self.sslCtx = SSL_CTX_new(TLSv1_2_method())
        guard let sslCtx = self.sslCtx
            else {
                return
        }
    ...
    public func beginSSL(timeout: Double, closure: Bool -> ()) {
        guard self.fd.fd != invalidSocket
            else {
                return
        }
        if self.ssl == nil {
    ...
    public func userCertificateChainFile(cert: String) -> Bool {
        self.initSocket()
        guard let sslCtx = self.sslCtx
            else {
                return false
    ...
    public override func forEachAccept(callback: (NetTCP?) -> ()) {
        super.forEachAccept {
    ...
    override public func accept(timeoutSecnods: Double, callback: (NetTCP?) -> ()) throws {
        try super.accept(timeoutSecnods, callback: { [unowned self] (net: NetTCP?) in
            if let netSSL = net as? NetTCPSSL {
                netSSL.sslCtx = self.sslCtx
                netSSL.ssl = SSL_new(self.sslCtx!)
                SSL_set_fd(netSSL.ssl!, netSSL.fd.fd)
                self.finishAccept(timeoutSecnods, net: netSSL, callback: callback)
            }
            else {
                callback(net)
            }
