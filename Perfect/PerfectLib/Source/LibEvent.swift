...
// Wait for asocket or FD to become writeable
...
class LibEvent {
    ...
    private static var eventCallBack: PrimEventCallBack {
        let c: PrimEventCallBack = { (a, b, c) in
    ...
    func add(inout tv: timeval) -> Void {
        event_add(event!, &tv)
    }
    func add(timeout: Double) -> Void {
        if timeout == -1 {
            event_add(event!, nil)
        }
        else {
...
let EVLOOP_NO_EXIT_ON_EMPTY = Int32(0/*0x04*/) // not supported untill livevent 2.1
class LibEventBase {
    ...
    init() {
        ...
        // and it only happens from within the singular libevent loop, we can encure is it not called concurrently.
