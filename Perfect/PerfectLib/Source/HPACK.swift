...
class DynamicTable {
    ...
    func remove() -> HeaderField? {
        guard let removed = headerFields[tail]
            else {
                return nil
        }
        size -= removed.size
...
public class HPACKEncoder {
    ...
    public func encodeHeader(out: Bytes, name: [UInt8], value: [UInt8], sensitive: Bool = false, incrementalIndexing: Bool = true) throws {
        ...
        if capacity == 0 {
            ...
            if staticTableIndex == -1 {
                ...
                eoncodeInteger(out, mask: 0x80, n: 7, i: staticTableIndex)
            }
            return
        }
        ...
        if headerSize > capacity {
            ...
            eoncodeInteger(out, mask: 0x80, n: 7, i: index)
        }
        else {
            let staticTableIndex = StaticTable.getIndex(name, value: value)
            if staticTableIndex != -1 {
    ...
    func add(name: [UInt8], value: [UInt8]) -> Void {
        let headerSize = HeaderField.sizeOf(name, value: value)
    ...
    func eoncodeInteger(out: Bytes, mask: Int, n: Int, i: Int) {
        let nbits = 0xff >> (8 - n)
        if i < nbits {
    ...
    func encodeStringLiteral(out: Bytes, string: [UInt8]) throws {
        ...
        if huffmanLength < string.count {
            eoncodeInteger(out, mask: 0x80, n: 7, i: huffmanLength)
            try huffmanEncoderInstance.encode(out, input: Bytes(existingBytes: string))
        }
        else {
            eoncodeInteger(out, mask: 0x00, n: 7, i: string.count)
    ...
    func encodeLiteral(out: Bytes, name: [UInt8], value: [UInt8], indexType: IndexType, nameIndex: Int) throws {
        ...
        eoncodeInteger(out, mask: mask, n: prefixBits, i: nameIndex == -1 ? 0 : nameIndex)
        if nameIndex == -1 {
    ...
    func setMaxHeaderTableSize(out: Bytes, maxHeaderTableSize: Int) {
        ...
        ensureCapacity(0)
        eoncodeInteger(out, mask: 0x20, n: 5, i: maxHeaderTableSize)
    }
}
...
public class HPACKDecoder {
    ...
    enum State {
        case ReadHeaderRepresentation, ReadMaxDynamicTableSize, ReadIndexedHeader, ReadIndexedHeaderName, ReadLiteralHeaderNameLengthPrefix, ReadLiteralHeaderNameLength, ReadLiteralHeaderName, SkipLiteralHeaderName, ReadLiteralHeaderValueLengthPrefix, ReadLiteralHeaderValueLength, ReadLiteralHeaderValue, SkipLiteralHeaderValue
    }
    ...
    func insertHeader(headerListener: HeaderListener, name: [UInt8], value: [UInt8], indexType: IndexType) -> Void {
