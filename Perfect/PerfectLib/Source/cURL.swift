...
public class CURL {
    ...
    public init(dupeCurl: CURL) {
        if let copyForm = dupeCurl.curl {
            self.curl = curl_easy_duphandle(copyForm)
        }
        else {
            self.curl = curl_easy_init()
        }
    ...
    public func perform() -> (Bool, Int, [UInt8]?, [UInt8]?) {
        ...
        if msg != nil {
            ...
            guard msgResult == CURLE_OK
                else {
                    return (false, Int(msgResult.rawValue), nil, nil)
            }
            return (false, Int(msgResult.rawValue), self.headerBytes.count > 0 ? self.headerBytes : nil, self.bodyBytes.count > 0 ? self.bodyBytes : nil)
        }
        
        return (true, 0, self.headerBytes.count > 0 ? self.headerBytes : nil, self.bodyBytes.count > 0 ? self.bodyBytes : nil)
    }
