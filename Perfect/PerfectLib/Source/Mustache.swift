...
public class MustacheEvaluationContext {
    ...
    /// Initalize a new context given the map of values.
    ...
    /// Search for a value starting from the current context. If not found in the current context, the parent context will be seached. etc.
...
public class MustacheEvaluationOutputCollector {
    var output = [String]()
    var defaultEncodingFunc: (String) -> String = {
        $0.stringByEncodingHTML
    }
    public init() {}
...
public class MustachePartialTag: MustacheTag {
    ...
    public override func evaluate(context: MustacheEvaluationContext, collector: MustacheEvaluationOutputCollector) {
        guard let page = context.getCurrentFilePath()
            else {
...
public class MustachePragmaTag: MustacheTag {
    ...
    public func parsePragma() -> Dictionary<String, String> {
        var d = [String: String]()
        let commaSplit = tag.characters.split() {
            $0 == Character(",")
            }.map {
                String($0)
        }
        for section in commaSplit {
            let colonSplit = section.characters.split() {
                $0 == Character(":")
                }.map {
                    String($0)
            }
...
public class MustacheGroupTag: MustacheTag {
    ...
    func evaluatePos(context: MustacheEvaluationContext, collector: MustacheEvaluationOutputCollector) -> Void {
        let cValue = context.getValue(tag)
        if let value = cValue {
            // if it is a dictionary, the eval children with it as the new context
            ...
            case let lambda as (String, MustacheEvaluationContext
                ) -> String:
...
public class MustacheParser {
    ...
    func consumePossibleOpenDelimiter(index: Int) -> Bool {
        if index == openDelimiters.count {
            // we successfully encountered a full delimiter sequence
            return true
        }
        if let e = next() {
    ...
    func consumeSetDelimiters() throws {
        ...
        if e != nil {
            ...
            guard e != nil && ICU.isWhiteSpace(e!)
                else {
            ...
            guard e! == closeDelimiters[0] && consumePossibleCloseDelimiter(1)
                else {
                    throw MustacheError.SyntaxError(errorMsg)
            }
