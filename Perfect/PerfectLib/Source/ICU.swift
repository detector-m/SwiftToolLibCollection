...
/// This class provides a series of class functions which expose some of the ICU library's functionality
public class ICU {
    ...
    /// return true if the UnicodeScalar is an alpha-numeric character
    ...
    public static func parseDate(dateStr: String, format: String, timezone inTimezone: String? = nil, locale inLocale: String? = nil) throws -> Double {
        ...
        guard U_SUCCESS(status)
            else {
                try ThrowICUError(status)
        }
        defer {
            udat_close_wrapper(dateFormat)
        }
    ...
    /// Format a date value according to the indicated format string and return a gate string.
    ...
    public static func formatDate(date: Double, format: String, timezone inTimezone: String? = nil, locale inLocale: String? = nil) throws -> String {
        ...
        if let tz = inTimezone {
            let tzUtf16 = tz.utf16
            timezone = UnsafeMutablePointer<UInt16>([UInt16](tzUtf16))
            timeZoneLength = Int32(tzUtf16.count)
        }
        if let loc = inLocale {
            let utf8Chars = loc.utf8
        ...
        let dateFormat = udat_open_wrapper(UDAT_PATTERN, UDAT_PATTERN, locale, timezone, timeZoneLength, [UInt16](utf16Chars), Int32(utf16Chars.count), &status)
        guard U_SUCCESS(status)
            else {
                try ThrowICUError(status)
        }
