...
public class NetNamedPipe: NetTCP {
    ...
    public func bind(address: String) throws {
        ...
        defer { addrPtr.destroy(); addrPtr.dealloc(addrLen) }
        var memLoc = 0
        ...
        addrPtr[memLoc] = 0
        #if os(Linux)
            let bRes = SwiftGlibc.bind(fd.fd, UnsafeMutablePointer<sockaddr>(addrPtr), socklen_t(addrLen))
        #else
    ...
    /// - parameter callback: the closure which will be called when the connectiong completes, If the connection completes successfully then the current NetNamedPipe instance will be passed to the callback, otherwise, a nil object will be passed.
    /// - returns: 
    ...
    /// - parameter callback: the callback call when the send completes. the parameter passed will be `true` if the send completed without error.
