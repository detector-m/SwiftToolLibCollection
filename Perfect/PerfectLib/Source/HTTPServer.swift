...
public class HTTPServer {
    ...
    /// This is important if untillizing the `WebRequest.serverName` "SERVER_NAME" variable.
    ...
    public func start(port: UInt16, sslCert: String, sslKey: String, dhParams: String? = nil, bindAddress: String = "0.0.0.0") throws {
        ...
        guard socket.userCertificateChainFile(sslCert)
            else {
    ...
    func runRequest(req: HTTPWebConnection) {
        guard let pathInfo = req.requestParams["PATH_INFO"]
            else {
    ...
    class HTTPWebConnection: WebConnection {
        ...
        func readBody(callback: OkCallback) {
            guard let cl = self.requestParams["CONTENT_LENGTH"]
            where Int(cl) > 0
                else {
                    return
            }
        ...
        func readBody(size: Int, callback: OkCallback) {
            ...
            self.connection.readSomeBytes(size) {
                ...
                if b == nil || b!.count == 0 {
                    self?.connection.readBytesFully(1, timeoutSecondes: httpReadTimeout) {
                        (b: [UInt8]?) in
                        guard b != nil
                            else {
                                callback(false)
        ...
        func processHeaderContinuation(h: ArraySlice<UInt8>) -> Bool {
            guard !self.lastHeaderKey.isEmpty
                else {
                    return false
            }
            guard let found = self.requestParams[self.lastHeaderKey]
                else {
                    return false
            }
            for i in 0..<h.count {
        ...
        func scanWorkingBuffer(callback: OkCallback) {
            ...
            for i in startingOffset..<self.workingBuffer.count {
                ...
                guard false == lastWasCr || httpLF == c
                    else {
                        // malformed header
                        callback(false)
                        return
        ...
        func didReadHeaderData(b: [UInt8]?, callback: OkCallback) {
            ...
            if b!.count == 0 {
                // no data was available for immediate consumption. try reading with timeout
                self.connection.readBytesFully(1, timeoutSecondes: httpReadTimeout) {
                    (b2: [UInt8]?) in
                    if b2 == nil {
                        // timeout, request dead
                        callback(false)
                    }
                    else {
