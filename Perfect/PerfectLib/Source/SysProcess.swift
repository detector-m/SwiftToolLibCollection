...
public class SysProcess: Closeable {
    ...
    public init(_ cmd: String, args: [String]?, env: [(String, String)]?) throws {
        ...
        defer {
            cArgs.destroy()
            cArgs.dealloc(cArgsCount + 2)
        }
        
        cArgs[0] = strdup(cmd)
        cArgs[cArgsCount + 1] = nil//UnsafeMutablePointer<CChar>(())
        var idx = 0
        ...
        defer {
            cEnv.destroy()
            cEnv.dealloc(cEnvCount + 1)
        }
        
        cEnv[cEnvCount] = nil //UnsafeMutablePointer<CChar>(())
        idx = 0
        while idx < cEnvCount {
        ...
        var action = posix_spawn_file_actions_t(nil)//posix_spawn_file_actions_t()
        ...
        //
        var procPid = pid_t()
        let spawnRes = posix_spawnp(&procPid, cmd, &action , nil/*UnsafeMutablePointer<posix_spawnattr_t>(())*/, cArgs, cEnv)
        ...
        self.stdout = File(fd: fSTDERR[0], path: "")
    }
