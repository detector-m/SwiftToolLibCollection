...
class FastCGIRequest: WebConnection {
    ...
    func makeEndRequestBody(requestId: Int, appStatus: Int, protocolStatus: Int) -> [UInt8] {
        ...
        b.import8Bits(fcgiVersion1).import8Bits(fcgiEndRequest).import16Bits(htons(UInt16(requestId))).import16Bits(htons(UInt16(8))).import8Bits(0).import8Bits(0).import32Bits(htonl(UInt32(appStatus))).import8Bits(UInt8(protocolStatus)).import8Bits(0).import8Bits(0).import8Bits(0)
    ...
    func makeStdoutBody(requestId: Int, data: [UInt8], firstPos: Int, count: Int) -> [UInt8] {
        ...
        if count > fcgiBodyChunkSize {
            ...
            b.import8Bits(fcgiVersion1).import8Bits(fcgiStdout).import16Bits(htons(UInt16(requestId))).import16Bits(htons(UInt16(count))).import8Bits(UInt8(padbytes)).import8Bits(0)
    ...
    func readRecord(continuation: (FastCGIRecord?) -> ()) {
        self.connection.readBytesFully(fcgiBaseRecordSize, timeoutSecondes: fcgiTimeoutSeconds) {
    ...
    func readRecordContent(record: FastCGIRecord, continuation: (FastCGIRecord?) -> ()) {
        if record.contentLength > 0 {
            self.connection.readBytesFully(Int(record.contentLength), timeoutSecondes: fcgiTimeoutSeconds, completion: { [weak self] (b: [UInt8]?) -> () in
    ...
    func readRecordPadding(record: FastCGIRecord, continuation: (FastCGIRecord?) -> ()) {
        if record.paddingLength > 0 {
            self.connection.readBytesFully(Int(record.paddingLength), timeoutSecondes: fcgiTimeoutSeconds, completion: { (b: [UInt8]?) -> () in
