...
#if USE_LIBDISPATCH
    // IFIX! This code no longer supports libdispatch and needs updating for Lock, Event and RWLock
    import Dispatch
#else
    #if os(Linux)
        import SwiftGlibc
...
// A wrapper around a variety of threading related function and classes
public class Threading {
    ...
    public class Event: Lock {
        ...
        deinit { pthread_cond_destroy(&cond) }
        ...
        // Return true only if the signal was received.
        // Return false upon timeout or error
    ...
    public class RWLock {
        ...
        /// Attempt to acquire the lock for reading.
        // return false if the lock is held by a writer or an error occurs.
    ...
    public static func dispatchBlock(closure: ThreadClosure) {
        ...
        #else
            var thrdSlf = pthread_t(nil) // pthread_t()
...
//        #else
//            return Threading.createConcurrentQueue(named)
//        #endif
//    }
    ...
    /// This is userful for ensuring that initialization code is only called once in a multi-threaded process.
