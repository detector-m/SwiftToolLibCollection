...
public class SQLite: Closeable {
    ...
    // Return the value of  `sqlite3_total_changes`.
    ...
    public func changes() -> Int {
        return Int(sqlite3_changes(self.sqlite3))
    }
    
    /// 
    public func errCode() -> Int {
        return Int(sqlite3_errcode(self.sqlite3))
    }
    ...
    public func execute(statement: String) throws {
        try forEachRow(statement, doBindings: {
                (SQLiteStmt) throws -> () in ()
            }, handleRow: { (SQLiteStmt) in
                // nothing
        })
    }
    ...
    /// Executes a BEGIN, calls the provided closure and executes a ROLLABCK if an exception occurs or a COMMIT if no exception occurs.
    ...
    func forEachRowBody(stat: SQLiteStmt, handleRow: (SQLiteStmt, Int) -> ()) throws -> Void {
        var r = stat.step()
        if r == SQLITE_LOCKED || r == SQLITE_BUSY {
            ...
            guard r != SQLITE_LOCKED && r != SQLITE_BUSY
                else {
                    try checkRes(r)
                    return
            }
    ...
    func miniSleep(millis: Int) -> Void {
        var tv = timeval()
...
public class SQLiteStmt: Closeable {
    ...
    public func columnCount() -> Int {
        return Int(sqlite3_column_count(self.stat!))
    }
    ...
    /// Returns the double value for the indicated
