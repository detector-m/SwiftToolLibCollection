...
public class WebSocket {
    ...
    public enum OpcodeType: UInt8 {
        case Continuation = 0x0, Text = 0x1, Binary = 0x02, Close = 0x8, Ping = 0x9, Pong = 0xa, Invalid
    }
    ...
    private func clearFrame() {
        let postion = self.readBuffer.position
        self.readBuffer.data.removeFirst(postion)
        self.readBuffer.position = 0
    }
    
    private func fillFrame() -> Frame? {
        guard self.readBuffer.availableExportBytes >= 2
            else {
                return nil
        }
        // we know we potentially have a valid frame here
        // for to be resetting the position if we don't hava a valid frame yet
    ...
    func fillBuffer(demand: Int, completion: (Bool) -> ()) -> Void {
        self.socket.readBytesFully(demand, timeoutSecondes: self.readTimeoutSeconds) {
    ...
    private func readFrame(completion: (Frame?) -> ()) {
        ...
        self.fillBuffer(1) {
            ...
            self.fillBufferSome(1024 *  32) {
                // some arbitrary read-ahead amount
                self.readFrame(completion)
            }
        }
    }
    ...
    /// Send binary data to hte client.
    ...
    public func sendPing(completion: () -> ()) {
        self.sendMessage(.Ping, bytes: [], final: true, completion: completion)
    }
    
    private func sendMessage(opcode: OpcodeType, bytes: [UInt8], final: Bool, completion: () -> ()) {
        let sendBuffer = Bytes()
        
        let byte1 = UInt8(final ? 0x80: 0x00) | (self.nextIsContinuation ? 0 : opcode.rawValue)
        ...
        if payloadSize < smallPayloadSize {
            ...
            sendBuffer.import8Bits(UInt8(smallPayloadSize)).import16Bits(htons(UInt16(payloadSize)))
        }
        else {
            sendBuffer.import8Bits(UInt8(1 + smallPayloadSize)).import64Bits(htonll(UInt64(payloadSize)))
        }
...
public protocol WebSocketSessionHandler {
    /// Optionally indicate the name of the protocol thehandler implements.
    ...
    /// This function is called once the WebSocket session has been initialted.
...
public class WebSocketHandler: RequestHandler {
    ...
    public func handleRequest(request: WebRequest, response: WebResponse) {
        guard let upgrade = request.headers["Upgrade"], connection = request.headers["Connection"], secWebSocketKey = request.headers["Sec-WebSocket-Key"], secWebSocketVersion = request.headers["sec-WebSocket-Version"] where upgrade.lowercaseString == "websocket" && connection.lowercaseString.contains("upgrade")
            else {
                response.setStatus(400, message: "Bas Request")
                response.requestCompletedCallback()
                return
        }
        guard acceptableProtocolVersions.contains(Int(secWebSocketVersion) ?? 0)
            else {
