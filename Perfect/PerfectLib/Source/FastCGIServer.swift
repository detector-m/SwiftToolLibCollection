...
/// When a request is received, the server will instantiate a `WebRequest/WebResponse` pair and the will handle the remainder of the request
public class FastCGIServer {
    ...
    func handleConnection(net: NetTCP) -> Void {
    ...
    func handleRecord(fcgiReq: FastCGIRequest, fcgiRecord: FastCGIRecord) {
        ...
        case fcgiStdin:
            if fcgiRecord.contentLength > 0 {
                ...
                // done initiating the request. run with it
                runRequest(fcgiReq)
                return
            }
        case fcgiData:
        ...
        case fcgiXStdin:
            if Int(fcgiRecord.contentLength) == sizeof(UInt32) {
                ...
                let size = ntohl((four << 4) + (three << 16) + (two << 8) + one)
    ...
    func readXStdin(fcgiReq: FastCGIRequest, size: Int) -> Void {
