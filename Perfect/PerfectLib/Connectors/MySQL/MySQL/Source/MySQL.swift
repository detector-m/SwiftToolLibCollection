...
public final class MySQL {
    ...
    // An empty (but not nil) string wolud have a count of 1
    static func convertString(s: String?) -> (UnsafeMutablePointer<Int8>, Int) {
        ...
        guard let notNilString = s
            else {
                return ret
        }
        notNilString.withCString {
            p in
            var c = 0
            while p[c] != 0 {
                c += 1
    ...
    func cleanConvertedString(pair: (UnsafeMutablePointer<Int8>, Int)) -> Void {
        if pair.1 > 0 {
            pair.0.destroy(); pair.0.dealloc(pair.1)
        }
    }
    ...
    public final class Results: GeneratorType {
        ...
        public func forEachRow(callback: (Element) -> ()) {
            while true {
                ...
                let lengths = mysql_fetch_lengths(self.ptr)
                var ret = [String]()
                for fieldIdex in 0..<self.numFields() {
                    let len = Int(lengths[fieldIdex])
                    let raw = UnsafeMutablePointer<UInt8>(row[fieldIdex])
                    let s = UTF8Encoding.encode(GenerateFromPointer(from: raw, count: len))
                    ret.append(s)
                }
                callback(ret)
...
public final class MySQLStmt {
    ...
    public func execute() -> Bool {
        if self.paramBindsOffset > 0 {
            guard 0 == mysql_stmt_bind_param(self.ptr, self.paramBinds)
                else {
                    return false
            }
        }
    ...
    public func paramCount() -> Int {
        return Int(mysql_stmt_param_count(self.ptr))
    }
    ...
    public final class Results: GeneratorType {
        ...
        func bindBuffer<T>(sourceBind: MYSQL_BIND, type: T) -> MYSQL_BIND {
            var bind = sourceBind
            bind.buffer = UnsafeMutablePointer<Void>(UnsafeMutablePointer<T>.alloc(1))
        ...
        public func forEachRow(callback: Element -> ()) -> Bool {
            ...
            for i in 0..<numFields {
                ...
                binds.advancedBy(1).initialize(bind)
            }
            ...
            guard 0 == mysql_stmt_bind_result(self.stmt.ptr, binds)
                else {
                    return false
            }
            while true {
