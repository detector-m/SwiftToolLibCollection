...
public struct MongoQueryFlag: OptionSetType {
    ...
    static let AwaitData = MongoQueryFlag(MONGOC_QUERY_EXHAUST)
...
public class MongoIndexOptions {
    ...
    // guard asainst the case where these values were set to something in the defaults.
    ...
    public init(name: String? = nil, background: Bool? = nil, unique: Bool? = nil, dropDups: Bool? = nil, sparse: Bool? = nil, expireAfterSeconds: Int32? = nil, v: Int32? = nil, defaultLanguage: String? = nil, languargeOverride: String? = nil, weights: BSON? = nil, geoOptions: MongoIndexOptionsGeo? = nil, storageOptions: MongoIndexStorageOptionType? = nil) {
        ...
        if let languargeOverride = languargeOverride {
            self.langOverNil = true
            self.rawOpt.language_override = UnsafePointer<Int8>(strdup(languargeOverride))
        }
        if let weigths = weights {
            self.weightsDoc = weights // reference this so the ptr doesn't disappear beneath us
            self.rawOpt.weights = UnsafePointer<bson_t>(weigths.doc)
        }
    ...
    deinit {
        if self.nameNil && self.rawOpt.name != nil {
            free(UnsafeMutablePointer<Void>(self.rawOpt.name))
        }
...
public class MongoCollection {
    ...
    public func count(query: BSON, fields: BSON? = nil, flags: MongoQueryFlag = MongoQueryFlag.None, skip: Int = 0, limit: Int = 0, bathSize: Int = 0) -> Result {
        var error = bson_error_t()
